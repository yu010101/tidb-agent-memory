...
<!--
投稿前チェック:
[x] 実TiDB Cloud Serverless(Singapore)で make schema/ingest/bench 完走。TiDB hybrid recall@10=0.939(>vector0.844/全文0.922)
[x] §4 を実TiDB+baseline比較表に更新。レイテンシのネットワークRTT注記も追加
[x] 技術主張10項を独立ファクトチェック済(全文=early stages/ベクトル=明示betaなし に修正、RRF主張を範囲限定に修正)
[x] 実機で fts_match_word×window制約(ERROR 1221)を発見→SQL修正→§5に追記
[x] TiDB Cloud を実使用（加点）/ OGPカード作成済
[ ] GitHub公開後、リポジトリURLを冒頭に差し込み / Zenn公開時にサムネ確認
-->
